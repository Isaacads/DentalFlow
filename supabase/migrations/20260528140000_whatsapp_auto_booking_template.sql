alter table public.clinics
  add column if not exists whatsapp_auto_booking_template text not null default
    'Olá, {nome}! Sua consulta foi agendada com sucesso para {data} às {hora}. Se precisar reagendar, responda esta mensagem.';

update public.clinics
set whatsapp_auto_booking_template = coalesce(nullif(whatsapp_auto_booking_template, ''), whatsapp_confirmation_template)
where whatsapp_auto_booking_template is null or whatsapp_auto_booking_template = '';
...
begin
  ...
  select coalesce(c.whatsapp_auto_booking_template, c.whatsapp_confirmation_template, 'Olá, {nome}! Sua consulta foi agendada com sucesso para {data} às {hora}.')
  into v_template
  from public.clinics c
